...
// been mounted via aurora_dvd_open() -- and that's compiled out entirely
// under Emscripten (src/game/main.c, `#ifndef EMSCRIPTEN`), since there's no
// disc image on this port, only the extracted/preloaded file tree. These
// override MSM_INIT's open/read/close hooks with plain fopen/fread against
// that preloaded tree instead, bypassing the DVD entrynum lookup for the
// audio system's own file (see the matching msmsys.c change that skips the
// entrynum check when a custom open hook is provided).
static BOOL MsmFileOpen(s32 entrynum, DVDFileInfo *fileInfo)
{
    FILE *f;
    (void)entrynum;
    f = fopen("/GMPE01_00/files/sound/mpgcsnd.msm", "rb");
    if (f == NULL) {
        return FALSE;
    }
    fileInfo->cb.userData = f;
    return TRUE;
}

static BOOL MsmFileRead(DVDFileInfo *fileInfo, void *addr, s32 length, s32 offset, s32 prio)
{
    FILE *f = (FILE *)fileInfo->cb.userData;
    (void)prio;
    if (f == NULL || fseek(f, offset, SEEK_SET) != 0) {
        return -1;
    }
    if ((s32)fread(addr, 1, (size_t)length, f) != length) {
        return -1;
    }
    return length;
}

static BOOL MsmFileClose(DVDFileInfo *fileInfo)
{
    FILE *f = (FILE *)fileInfo->cb.userData;
    if (f != NULL) {
        fclose(f);
        fileInfo->cb.userData = NULL;
    }
    return TRUE;
}
...
void HuAudInit(void)
{
    ...
    // Streamed audio (compType 4/5, played via msmStreamPlay) isn't
    // implemented by the software mixer (extern/musyx/src/musyx/runtime/
    // hw_pc.c) -- it depends on ARAM DMA, which is a no-op stub on this
    // target. Passing pdtPath=NULL makes msmStreamInit() (src/msm/
    // msmstream.c) skip straight to "no streaming" instead of also hitting
    // the DVD entrynum problem below.
    msmInit.pdtPath = NULL;
    msmInit.open = MsmFileOpen;
    msmInit.read = MsmFileRead;
    msmInit.close = MsmFileClose;
    msmAram.skipARInit = TRUE;
    msmAram.aramEnd = 0x808000;
    result = msmSysInit(&msmInit, &msmAram);

    if (result < 0) {
        // OSReport() is silent under Emscripten (src/port/stubs.c), so use
        // printf directly. The original engine halts here (while(1)) since
        // on real hardware a failed audio init is unrecoverable -- on this
        // port that would just freeze the whole page, so fail soft instead:
        // skip audio setup and let the game continue silently.
...
void HuAudSndGrpSet(s16 grpId) {
    ...
    buf = HuMemDirectMalloc(HEAP_DATA, msmSysGetSampSize(FALSE));
...
void HuAudSndCommonGrpSet(s16 grpId, s32 groupCheck) {
    ...
    buf = HuMemDirectMalloc(HEAP_DATA, msmSysGetSampSize(TRUE));
